...
public class sc_SpaceShip_IA01_ToCompare_TODELETE : MonoBehaviour
{
    [Header("Parameters")]
    ...
    public bool _isLeaded = false;
    public GameObject sLeader = null;
    public int squadMaxLength = 5;
    int squadLength = 0;
    [SerializeField] public List<GameObject> squadList;
    ...
    void AssignPositionsInSquad (GameObject[] squadMates)
    {
        ...
        for (int i = 1; i < squadMates.Length; i++)
        {
            ...
            {
                squadMates[i].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>()._target = mySquadPoints.ToArray()[i].transform;
                squadMates[i].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>().squadTarget = mySquadPoints.ToArray()[i].transform;
            }
        }
    ...
    void CheckForLeader (GameObject[] squadMembers)
    {
        ...
        {
            if (friendList.ToArray()[i].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>()._squadLeader == true)
            {
                leaderPresent = true;
                break;
            }
    ...
    {
        friendList.ToArray()[0].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>()._squadLeader = true;
        sLeader = friendList.ToArray()[0];
        sLeader.name = ("Captain " + 0);

        for (int i = 0; i < friendList.ToArray().Length; i++)
        {
            friendList.ToArray()[i].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>().leaderPresent = true;
            friendList.ToArray()[i].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>().sLeader = sLeader;
        }
    }
    ...
    void EstablishSquad (GameObject[] fl)
    {
        for (int i = 0; i < fl.Length; i++)
        {
            if (fl.Length >= squadMaxLength)
            {
                if (squadLength < squadMaxLength)
                {
                    if (fl[i].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>()._isLeaded == false)
                    {
                        squadList.Add(fl[i]);
                        fl[i].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>()._isLeaded = true;
                        squadLength++;
                    }
                }
                else
    ...
    void GetShipList ()
    {
        ...
        {
            if (shipList[i].GetComponent<sc_SpaceShip_IA01_ToCompare_TODELETE>()._myFaction != _myFaction)
            {
        ...
        EstablishSquad(friendList.ToArray());
    }
