...
public class DisplayChanger : MonoBehaviour
{
    ...
    public void ButtonChangeDisplay(int disp)
    {
    ...
    public void ButtonSurroundScreen()
    {
        ...
        if (Surround == false)
        {
            if (monitorNumber < myDisplays.Count)
            {
                StartCoroutine(MyMoveWindow(myMonitors[monitorNumber].height, totalWidth, myMonitors[monitorNumber].targetX, myMonitors[monitorNumber].targetY));
                Surround = true;
            }
        }
        else
    ...
    public IEnumerator MyMoveWindow(int newHeight, int newWidth, int targetX, int targetY)
    {
        IntPtr hwnd;
        RECT Rect = new RECT();
